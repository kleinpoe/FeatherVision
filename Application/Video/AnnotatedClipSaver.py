...
class AnnotatedClipSaver:
    ...
    def Save(self, timestamp:datetime, entries: list[DetectionHistoryEntry]):
        ...
        self.logger.debug(f'Writing annotated Clip: W{width}xH{height} FPS={estimatedFps:.1f} Frames={len(annotatedFrames)} Duration={(entries[-1].Timestamp-entries[0].Timestamp)}')
